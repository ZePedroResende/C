...
void main() {
    ...
    while (1) //infinite while loop for editing multiple number of tiles
    {
        ...
        printf("R :opens a file and reads it into a buffern   If file doesnot exist creates a new file for editing\n");
...
void inp(void) {
    ...
    {
        printf("\nThere is another file open it will be closedndo you want to continue ?(Y/N):");
        c = getch();
        if (c == 'n' || c == 'N')
        return;
        else
...
void edit(void) //the edit function
{
    ...
    printf("\nThe file contents will be displayed along with the line numbernpress any key\n");
    getch();
    system("clear");
